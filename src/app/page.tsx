...
export default function HomePage() {
  ...
  const featuredSites = CANONICAL_HERITAGE_SITES.filter(s => s.featured).slice(0, 6);
  const upcomingWorkshops = CANONICAL_WORKSHOPS.slice(0, 3);
  const verifiedGuides = CANONICAL_GUIDES.slice(0, 3);
  const sweetsAndFoods = CANONICAL_FOOD_ITEMS.slice(0, 4);
  const handicrafts = CANONICAL_PRODUCTS.slice(0, 4);
    ...
    <div className="flex flex-col w-full space-y-20 pb-20">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
            ...
            <div className="lg:col-span-7 space-y-6 text-left">
              ...
              <p className="text-base sm:text-lg text-stone-600 dark:text-stone-300 max-w-xl leading-relaxed">
                Connect with verified local guides, traditional craft masters, sweet merchants, and community guardians across 55+ canonical heritage sites of Bengal.
              </p>
              ...
              <div className="grid grid-cols-3 gap-4 pt-6 border-t border-stone-200 dark:border-stone-800">
                <div>
                  <div className="text-2xl font-black text-stone-900 dark:text-stone-100">55+</div>
                  <div className="text-xs text-stone-500 font-medium">Canonical Sites</div>
                </div>
                <div>
                  <div className="text-2xl font-black text-amber-600 dark:text-amber-400">100%</div>
                  <div className="text-xs text-stone-500 font-medium">Verified Guides</div>
                </div>
                <div>
                  <div className="text-2xl font-black text-stone-900 dark:text-stone-100">UNESCO</div>
                  <div className="text-xs text-stone-500 font-medium">3 World Treasures</div>
                </div>
              </div>
            </div>
            ...
            <div className="lg:col-span-5 relative">
              <div className="relative rounded-3xl overflow-hidden shadow-2xl border-4 border-white dark:border-stone-800 aspect-[4/5] group">
                <img 
                  src="https://images.unsplash.com/photo-1558431382-27e303142255?auto=format&fit=crop&w=1000&q=80" 
                ...
                <div className="absolute bottom-6 left-6 right-6 text-white space-y-2">
                  <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-amber-500 text-stone-950 text-xs font-bold">
                    <Award className="w-3.5 h-3.5" />
                    <span>Featured Landmark</span>
                  </div>
                  <h3 className="text-2xl font-extrabold text-white">Victoria Memorial, Kolkata</h3>
                  <p className="text-xs text-stone-300 line-clamp-2">
                    White Makrana marble masterpiece blending British neoclassical with Mughal domes, crowned by the Angel of Victory.
                  </p>
                  <div className="pt-2 flex items-center justify-between">
                    <span className="text-xs text-amber-300 font-semibold">Kolkata District</span>
                    <Link
              ...
              <div className="absolute -top-4 -left-4 bg-white dark:bg-stone-900 border border-stone-200 dark:border-stone-800 p-3 rounded-2xl shadow-xl flex items-center gap-3">
                ...
                <div>
                  <p className="text-xs font-bold text-stone-900 dark:text-stone-100">ASI & WB Tourism</p>
                  <p className="text-[10px] text-stone-500">Grounded Verified Registry</p>
                </div>
              </div>
            </div>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
          {[
            { title: 'Monuments & Forts', icon: '🏛️', count: '55 Sites', href: '/heritage' },
            { title: 'Living Festivals', icon: '🪔', count: 'Durga & Kali Puja', href: '/culture' },
            { title: 'Folk Music & Baul', icon: '🪕', count: 'Oral Minstrels', href: '/culture/baul-tradition' },
            { title: 'Ancient Crafts', icon: '🏺', count: 'Dokra & Terracotta', href: '/local-items' },
            { title: 'Handloom Silks', icon: '🧵', count: 'Baluchari Sari', href: '/local-items' },
            { title: 'Sweet Heritage', icon: '🍯', count: 'Banglar Rosogolla', href: '/food' }
          ].map((cat, i) => (
            <Link
              key={i}
              href={cat.href}
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-8 gap-4">
          <div>
            ...
            <p className="text-xs text-stone-500 mt-1">
              From the mangrove deltas of Sundarbans to the terracotta temples of Bishnupur and misty Darjeeling.
            </p>
          </div>
          <Link href="/heritage" className="text-xs font-bold text-amber-600 hover:text-amber-700 flex items-center gap-1">
            <span>View All 55 Sites</span>
            <ArrowRight className="w-4 h-4" />
          </Link>
        </div>
            ...
            >
              ...
              <div className="relative aspect-[16/10] overflow-hidden bg-stone-100 dark:bg-stone-800">
                ...
                <div className="absolute top-3 left-3 flex gap-2">
                  <span className="px-2.5 py-1 rounded-full text-[11px] font-bold bg-white/90 dark:bg-stone-900/90 text-stone-800 dark:text-stone-200 backdrop-blur-md">
                    {site.district}
                  </span>
                </div>
                <button
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-8 gap-4">
            <div>
              ...
              <p className="text-xs text-stone-500 mt-1">
                Learn lost-wax metallurgy, Patachitra painting, and Chhau mask-making directly from national awardees. Includes verified QR certificate!
              </p>
            </div>
              ...
              >
                <div>
                  <div className="relative aspect-[16/9] overflow-hidden">
                    ...
                    <span className="absolute top-3 left-3 px-2.5 py-1 rounded-full text-[10px] font-bold bg-amber-600 text-white shadow-sm">
                      {ws.category}
                    </span>
                  </div>
              ...
              <div className="space-y-4">
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-stone-500 font-medium">{guide.location}</p>
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-6 gap-4">
            <div>
              ...
              <h2 className="text-2xl font-extrabold text-stone-900 dark:text-stone-100 mt-1">
                GI-Tagged Sweets & Culinary Delights
              </h2>
            </div>
        ...
        <div className="p-8 rounded-3xl bg-white dark:bg-stone-900 border-2 border-amber-300/80 dark:border-amber-800/50 shadow-xl space-y-6">
          ...
          <div className="space-y-2">
            <h3 className="text-xl font-extrabold text-stone-900 dark:text-stone-100">
              In which year was Kolkata’s Durga Puja inscribed on the UNESCO Intangible Cultural Heritage list?
            </h3>
            <p className="text-xs text-stone-500">Test your knowledge grounded in verified archive documentation.</p>
          </div>

          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {['2015', '2019', '2021', '2023'].map((year, idx) => {
          ...
          {quickQuizAnswered && (
            <div className="p-4 rounded-2xl bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-800/40 text-xs text-amber-900 dark:text-amber-200 flex items-center justify-between animate-in fade-in">
              <p>
                {quickQuizSelected === 2 
                  ? '🎉 Correct! Durga Puja was inscribed in December 2021 as a triumph of public art.' 
                  : 'Almost! Durga Puja was officially inscribed in December 2021.'}
              </p>
              <Link href="/quiz" className="font-bold underline text-amber-700 dark:text-amber-300 ml-4 shrink-0">
                Play Full 8-Question Quiz →
              </Link>
            </div>
          )}
        </div>
